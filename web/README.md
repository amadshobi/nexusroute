# Goblin Nexus (GN) Web Dashboard

Web monitor & Mission Control visual untuk **Goblin Nexus Gateway** (`:4010`) yang menjembatani lalu lintas AI antara **OpenCode**, **Hermes Subagents**, **OMP Antigravity Broker** (`:4000`), dan **VansRouter Proxy** (`:20128`).
...
Dashboard ini dikompilasi menjadi aset statis murni di `dist/` dan dilayani secara **zero-runtime overhead** oleh Bun Server di Goblin Nexus:

```
[ Browser / HP ]  ──(HTTP :4010/dashboard/)──>  [ GN Bun Gateway Engine ]
                                                          │
                                         ┌────────────────┴────────────────┐
                                         ▼                                 ▼
...
cd tools-cli/src/gn/web
bun run build
systemctl --user restart gn-gateway.service
```
...
1. Pastikan port `4010` sudah diizinkan di Windows Firewall:
   ```powershell
   New-NetFirewallRule -DisplayName "Goblin Nexus Port 4010" -Direction Inbound -LocalPort 4010 -Protocol TCP -Action Allow
   ```
